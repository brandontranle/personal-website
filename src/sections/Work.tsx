...
export default function Work() {
  ...
  return (
    <div className="flex h-full flex-col py-2">
      ...
      <ul
        className="mt-6 flex min-h-0 flex-1 flex-col overflow-y-auto border-t border-white/[0.12]"
        ...
        {content.projects.map((p, i) => (
          <li key={p.title} onMouseEnter={() => setHover(i)}>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className={`grid grid-cols-[1fr_auto] items-center gap-4 border-b border-white/[0.12] px-1 py-4 transition-all hover:bg-white/[0.03] hover:pl-3 md:grid-cols-[300px_minmax(0,1fr)_210px_20px] md:gap-6 ${
                hover != null && hover !== i ? 'opacity-40' : 'opacity-100'
              }`}
            >
              <div className="flex flex-col gap-1">
                <span className="font-serif text-[26px] leading-none text-white">{p.title}</span>
